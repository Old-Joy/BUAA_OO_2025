...
public class MainClass {
    public static void main(String[] args) {
        ...
        if (first.equals("1")) {
            String simpleFunction = preprocessSigns(sc.nextLine().replaceAll("\\s+", ""));
            ArrayList<String> function = new ArrayList<>();
            function.add(simpleFunction);
            SimpleFuncCall.addDef(function);
        } else if (first.equals("2")) {
            String simpleFunctionFirst = preprocessSigns(sc.nextLine().replaceAll("\\s+", ""));
            String simpleFunctionSecond = preprocessSigns(sc.nextLine().replaceAll("\\s+", ""));
            ArrayList<String> function = new ArrayList<>();
            function.add(simpleFunctionFirst);
            function.add(simpleFunctionSecond);
            SimpleFuncCall.addDef(function);
        }
